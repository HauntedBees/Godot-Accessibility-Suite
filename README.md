...
`GASUtils.save_screen(name:String)` will save the current viewport texture to `user://save_{name}.png`, overwriting any existing file with that name. This can then be accessed with `GASUtils.get_screen_as_texture_rect(name:string)`, which returns a `TextureRect` node to be easily dropped into your game scene. `GASUtils.get_screen_as_texture` and `GASUtils.get_screen_as_image` also exist if you want the raw `ImageTexture` or `Image` instead. Both `get_screen_as_texture_rect` and `get_screen_as_texture` take additional optional arguments, `width:float`, `height:float`, and `keep_aspect_ratio:bool`, that allow you to resize the texture (by default the saved image is half the size of the viewport). Calling `GASUtils.get_screen_as_texture_rect("test", 320, 0, true)` will ensure the image keeps its original aspect ratio, with a maximum width of 320px. Likewise, `GASUtils.get_screen_as_texture_rect("test", 0, 320, true)` will keep the aspect ratio with a maximum height of 320px. `GASUtils.get_screen_as_texture_rect("test", 320, 320, false)` will resize the image to 320x320, regardless of its original aspect ratio. `GASUtils.get_screen_as_texture_rect("test", 320, 320, true)` will maintain the aspect ratio while ensuring that neither the width nor height exceed 320px.
