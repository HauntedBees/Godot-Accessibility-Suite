@tool
class_name GASVirtualGizmo extends Control

const EPSILON := 2.0
const SCALE_RATIO := 0.01
const MIN_SCALE := 0.5
var size_texture := preload("res://addons/gas/virtual_gamepad/parts/resize.png")
var move_texture := preload("res://addons/gas/virtual_gamepad/parts/reposition.png")
...
func _on_toggle_toggle() -> void:
	p.is_toggle = toggle_toggle.button_pressed

func _on_dynamic_toggle() -> void:
...
func _on_deadzone_change(v: float) -> void:
	p.dead_zone = v / 100.0

func _align_gizmo() -> void:
...
func _begin_action(action) -> void:
...
func _handle_move(event: InputEvent) -> void:
	...
	if !_configure_on_just_pressed_and_return_if_valid(event):
		return

	var delta: Vector2 = event.position - cursor_offset
	if delta.length() < EPSILON:
		return # don't move things around for tiny finger wiggles
	p.position += delta
	cursor_offset = event.position

func _handle_scale(event: InputEvent, dir: int) -> void:
	if !(event is InputEventMouseMotion || event is InputEventScreenTouch):
		return
	if !_configure_on_just_pressed_and_return_if_valid(event):
		return
