...
func _add_audit_entry(id: String, message: String, grade := GASAuditEntry.Grade.Failed) -> void:
	_notes.append(GASAuditEntry.new(id, _node.source_path, _node.path, message, grade))
